using System.Security.Cryptography.X509Certificates;
using System.ComponentModel.DataAnnotations;
using System.Drawing;

namespace HousePriceAPI.Models
{
    public class HousePrice
    {

      [Key] public int RegionID { get; set; }
        public string RegionName { get; set; }
        public string StateName { get; set; }
        public double Oct2021 { get; set; }
        public double Nov2021 { get; set; }
        public double Dec2021 { get; set; }
        public double Jan2022 { get; set; }
        public double Feb2022 { get; set; }
        public double Mar2022 { get; set; }
        public double Apr2022 { get; set; }
        public double May2022 { get; set; }
        public double Jun2022 { get; set; }
        public double Jul2022 { get; set; }
        public double Aug2022 { get; set; }
        public double Sep2022 { get; set; }
        public double Oct2022 { get; set; }
        public double Nov2022 { get; set; }
        public double Dec2022 { get; set; }
        public double Jan2023 { get; set; }
        public double Feb2023 { get; set; }
        public double Mar2023 { get; set; }
        public double Apr2023 { get; set; }
        public double May2023 { get; set; }
        public double Jun2023 { get; set; }
        public double Jul2023 { get; set; }
        public double Aug2023 { get; set; }
        public double Sep2023 { get; set; }
        public double Oct2023 { get; set; }



    }
}
